...
module.exports = function(logger, portalConfig, poolConfigs) {
  ...
  function setupStatsRedis() {
    ...
    redisStats.on("error", function(err) {
      logger.error(
        logSystem,
        "Historics",
        "Redis for stats had an error " + JSON.stringify(err)
      );
    });
  }
  ...
  function gatherStatHistory() {
    ...
    redisStats.zrangebyscore(["statHistory", retentionTime, "+inf"], function(
      err,
      replies
    ) {
      if (err) {
  ...
  this.getBlockHistory = async function() {
    let result = [];
    for (var i in redisClients) {
      var client = redisClients[i];
      var keys = await client.client.keysAsync("bitcoin:shares:round*");
      for (var j in keys) {
        var key = keys[j];
        var obj = await client.client.hgetallAsync(key);
        var addr = Object.keys(obj)[0];
        var time = 0;
        if ("time" in obj) {
          time = parseInt(obj["time"]);
        }
        var height = parseInt(key.replace("bitcoin:shares:round", ""), 10);
        var record = { height: height, addr: addr, time: time };
        result.push(record);
      }
    }
        ...
        client.client.multi(redisCommands).exec(function(err, replies) {
          if (err) {
            logger.error(
              logSystem,
              "Global",
              "error with getting global stats " + JSON.stringify(err)
            );
            callback(err);
          } else {
      ...
      function(err) {
        ...
        Object.keys(allCoinStats).forEach(function(coin) {
          ...
          for (var worker in coinStats.workers) {
            coinStats.workers[
              worker
            ].hashrateString = _this.getReadableHashRateString(
          ...
          .exec(function(err, replies) {
            if (err)
              logger.error(
                logSystem,
                "Historics",
                "Error adding stats to historics " + JSON.stringify(err)
              );
          });
        callback();
      }
